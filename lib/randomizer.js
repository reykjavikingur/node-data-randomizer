...
class Randomizer {
	...
	composites(branchCount, maxDepth, recursivePropertyName, baseStructure) {
		...
		assert(typeof maxDepth === 'number', argErrorMessage);
		...
		if (maxDepth > 0) {
			let randomChildren = this.arrays(branchCount, this.composites(branchCount, maxDepth - 1, recursivePropertyName, baseStructure));
	...
	transformations(factories, f) {
		if (!factories || !f) {
			throw new Error('invalid or missing arguments: should be (Array<Function>, Function)');
		}
		for (let factory of factories) {
			if (typeof factory !== 'function') {
				throw new Error('invalid argument factory');
			}
		}
		return ()=> {
			this.group();
			let args = factories.map((factory) => factory());
			this.ungroup();
