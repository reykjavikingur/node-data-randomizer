...
class Randomizer {
	...
	composites(branchCount, depth, recursivePropertyName, baseStructure) {
		...
		if (typeof depth === 'function') {
			assert(typeof depth.shift === 'function', 'function value for `depth` argument must be generated from `integers` method');
		}
		...
		// FIXME there is no point when we are actually calling `depth` so make it into an ad hoc factory

		let randomChildren = () => [];
		if (typeof depth === 'function') {
			let nextDepth = depth.shift(-1, 0);
			if (Boolean(nextDepth)) {
				randomChildren = this.arrays(branchCount, this.composites(branchCount, nextDepth, recursivePropertyName, baseStructure));
			}
		}
		else if (typeof depth === 'number') {
			if (depth > 0) {
				randomChildren = this.arrays(branchCount, this.composites(branchCount, depth - 1, recursivePropertyName, baseStructure));
			}
		}
