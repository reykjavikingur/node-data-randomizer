...
class Randomizer {
	...
	composites(branchCount, depth, recursivePropertyName, baseStructure) {
		...
		if (typeof depth === 'number') {
			depth = this.integers(depth, depth);
		}

		return this.transformations([randomBase, depth], (base, d) => {
			let randomChildren = () => [];
			if (d > 0) {
	...
	permutations(count, list) {
		return () => {
			...
			while (perm.length < n && reserve.length > 0) {
				let index = this.integers(0, reserve.length - 1)();
